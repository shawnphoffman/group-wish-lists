...
interface PostChristmasEmailProps {
	name: string
	items: { title: string; image_url: string; gifters: string[] }[]
	addons?: { description: string; gifters: string[] }[]
}

export default function PostChristmasEmail({ name, items, addons = [] }: PostChristmasEmailProps) {
	return (
		<Html>
			<Head />
			{/* <Preview>A look back at your Christmas gifts, {name}!</Preview> */}
			<Tailwind>
				<Body className="px-2 mx-auto my-auto font-sans bg-black dark`">
					<Container className="mx-auto my-[40px] max-w-[650px] rounded border bg-white border-[#eaeaea] border-solid p-[20px]">
						...
						<Heading className="mx-0 my-[20px] p-0 font-bold text-[24px] text-black text-center">
							A look back at your Christmas haul...
						</Heading>
						<Text className="text-base text-center">
							Here&apos;s a quick reference of some of the items that you were gifted this Christmas.
						</Text>
						<Hr />
						{items.length > 0 && (
							<>
						...
						{addons.length > 0 && (
							<>
								<Heading className="mx-0 my-[20px] p-0 font-bold text-[20px] text-black text-center">Addons</Heading>
								<Text className="text-sm text-center text-gray-600 mb-4">Additional gifts and notes that were added to your list.</Text>
								{addons.map((addon, index) => (
									<Section key={index}>
										<Row className="flex flex-row items-center justify-center w-full">
											<Column className="gap-2">
												<Text className="my-0 text-base font-bold leading-tight">{addon.description}</Text>
												<Text className="my-0 text-sm">From: {addon.gifters.join(', ')}</Text>
											</Column>
										</Row>
										<Hr />
									</Section>
...
PostChristmasEmail.PreviewProps = {
	name: 'Shawn',
	items: [
		{
			title:
